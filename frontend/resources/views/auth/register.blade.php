<x-auth.layout>
    <div class="rounded border border-zinc-200 bg-white p-4">
        <h1 class="mb-4 text-sm font-semibold">Register</h1>

        <form method="POST" action="/register" class="space-y-3">
            @csrf

            <div>
                <label class="block text-xs font-medium text-zinc-600">Name</label>
                <input name="name" value="{{ old('name') }}" class="mt-1 w-full rounded border border-zinc-300 px-3 py-2 text-sm" />
                @error('name')
            ...
            <div class="flex items-center justify-between gap-2">
                <button class="rounded bg-zinc-900 px-3 py-2 text-sm text-white hover:bg-zinc-800" type="submit">Register</button>
                <a class="text-sm text-zinc-600 hover:underline" href="/login">I already have an account</a>
            </div>
        </form>
    </div>
</x-auth.layout>
